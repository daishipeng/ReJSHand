...
class SemanticGraphCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(SemanticGraphCNN, self).__init__()
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, channels, num_heads):
        ...
        self.values = nn.Linear(channels, channels)
        self.keys = nn.Linear(channels, channels)
        self.queries = nn.Linear(channels, channels)
        self.fc_out = nn.Linear(channels, channels)

    def forward(self, x):
        N, L, C = x.shape  # (batch_size, sequence_length, channels)
        
        values = self.values(x).view(N, L, self.num_heads, self.head_dim)
        keys = self.keys(x).view(N, L, self.num_heads, self.head_dim)
        queries = self.queries(x).view(N, L, self.num_heads, self.head_dim)

        attention = torch.einsum("nqhd,nkhd->nhqk", [queries, keys]) * self.scale
        attention = F.softmax(attention, dim=-1)

        out = torch.einsum("nhql,nlhd->nqhd", [attention, values]).reshape(N, L, C)
        out = self.fc_out(out)

        return out

class FeatureChatBlock(nn.Module):
    def __init__(self, in_channels):
        super(FeatureChatBlock, self).__init__()
        self.sgcn = SemanticGraphCNN(in_channels, in_channels)
        self.mhsa = MultiHeadSelfAttention(in_channels, num_heads=8)
        self.fc = nn.Linear(in_channels, in_channels)

    def forward(self, joint_features):
        skeleton_features = joint_features
        # Update joint features using SGCN
        joint_features = joint_features.permute(0, 2, 1)  # (batch_size, channels, sequence_length)
        joint_features = self.sgcn(joint_features)
        joint_features = joint_features.permute(0, 2, 1)  # (batch_size, sequence_length, channels)
        
        # Update joint features using MHSA
        joint_features = self.mhsa(joint_features)
        
        # Feature interaction using Transformer's qkv mechanism
        joint_features = joint_features + skeleton_features  # Simplified interaction

        # Refine the joint features through a simple FC layer
        joint_features = self.fc(joint_features)

        # Add residual connection
        residual = joint_features + skeleton_features
...
class MeshHead(nn.Module):
    def __init__(
        ...
        self.encoder_1 = FeatureChatBlock(256)
        ...
        self.encoder_2 = FeatureChatBlock(128)
        ...
        self.encoder_3 = FeatureChatBlock(64)
        ...
        # self.mha = MultiHeadSelfAttention(dim=64, num_heads=4)
    ...
    def forward(self, feat: Tensor, uv_coords: Tensor) -> Tensor:
        ...
        # token_features = token_features + features_reshape1
        ...
        # token_features = token_features + features_reshape2
