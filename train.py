...
def count_parameters_and_memory(model):
    ...
    total_memory = total_params * 4  # assuming 32-bit (4 bytes) precision for each parameter

    print(f"Total parameters: {total_params}")
    print(f"Total memory: {total_memory / (1024 ** 2):.2f} MB")  # convert bytes to megabytes
...
class Trainer:
    ...
    def before_train(self):
        ...
        if self.rank == 0:
            logger.add("./train_log/out_dsp_2.log", backtrace=True, diagnose=True)   
        self.set_random_seed(5)
        ...
        self.dump_epoch_interval = self.cfg["TRAIN"]["DUMP_EPOCH_INTERVAL"] # 5
        ...
        print(count_parameters_and_memory(model))
        device = 'cuda'
        model.to(device)
