...
class HandNet(nn.Module):
    def __init__(self, cfg, pretrained=None):
        ...
        if "fastvit_ma36" in backbone_cfg['model_name']:
            pretrained_cfg_overlay = {'file' : r"/home/asrock/quanbao/daishipeng/simpleHand/weight/fastvit/pytorch_model.bin"}
            self.backbone = timm.create_model(backbone_cfg['model_name'], pretrained_cfg_overlay=pretrained_cfg_overlay, pretrained=pretrained, drop_path_rate=backbone_cfg['drop_path_rate'])
            self.is_hiera = False
        else:
            pretrained_cfg_overlay = {'file' : r"/home/asrock/quanbao/daishipeng/simpleHand/weight/repvit/pytorch_model.bin"}
            self.backbone = timm.create_model(backbone_cfg['model_name'], pretrained_cfg_overlay=pretrained_cfg_overlay, pretrained=pretrained)
            self.is_hiera = False
